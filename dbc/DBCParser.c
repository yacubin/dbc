...
#define isSpaceSymbol(c) ((c) == ' ' || (c) == '\t' || (c) == '\v')i
#define isASCIILower(c) ('a' <= (c) && (c) <= 'z')
#define isASCIIUpper(c) ('A' <= (c) && (c) <= 'Z')
#define isASCIIAlpha(c) (isASCIILower((c) | 0x20))
#define isFirstCIdentifier(c) (isASCIIAlpha(c) || (c) == '_')
#define isASCIIDigit(c) ('0' <= (c) && (c) <= '9')
#define isNextCIdentifier(c)  (isASCIIDigit(c) || isFirstCIdentifier(c))
#define isDoubleSymbol(c) (isASCIIDigit(c) || (c) == '.' || (c) == 'e' || (c) == 'E' || (c) == '+' || (c) == '-')

#ifndef ARRAY_SIZE
# define ARRAY_SIZE(a) (sizeof(a) / sizeof((a)[0]) / ((sizeof(a) % sizeof((a)[0])) ? 0 : 1))
#endif
...
{
  if (!isFirstCIdentifier(*s))
    return false;

  while (*s) {
    if (!isNextCIdentifier(*s))
      return false;
    s++;
  }
...
int32_t cstrToInt32(const char* str, char** end, int base)
{
  bool sign;
  char c = *str++;
  int32_t result = 0;
...
static uint32_t cstrToUint32(const char* str, char** end, int base)
{
  char c = *str++;
...
static bool DBCLexer_apply(DBCLexer* thiz, char c)
{
  ...
  case kLexerModeInit:
    ...
    case '+':
      if (thiz->indent == 0 && isASCIIDigit(thiz->prev))
  ...
  case kLexerModeUInteger:
    ...
    if (thiz->mode == kLexerModeUInteger && isNextCIdentifier(c)) {
      thiz->mode = kLexerModeCIdentifier;
      *thiz->position++ = c;
      return true;
    }
...
struct DBCOpcode {
  ...
  char required;
  const char* pattern;
  void (*init) (DBCMachine*, DBCSection*);
};
...
{
  DBC_ASSERT(thiz->teil < kDBCStackSize);
...
{
  DBC_ASSERT(thiz->head < thiz->teil);
  DBC_ASSERT(thiz->types[thiz->head] == type);
  (void)type;
...
static const DBCRange* DBCMachine_shiftRangeList(DBCMachine* thiz, size_t* size)
{
  size_t sz;
  const DBCRange* values = (const DBCRange*)DBCMachine_shiftUint32List(thiz, &sz);
  *size = sz / 2;
  DBC_ASSERT(*size * 2 == sz);
  return values;
}
...
static const DBCValDesc* DBCMachine_shiftDescriptions(DBCMachine* thiz, size_t* size)
{
  const struct DBCValDesc* values = (const DBCValDesc*)&thiz->values[thiz->head];
  size_t sz = DBCMachine_count(thiz);

#ifdef DBC_CPU_32BIT
  DBC_STATIC_ASSERT(sizeof(DBCValDesc[3]) == sizeof(uintptr_t) * 3 * 2, "");
#else
  DBC_STATIC_ASSERT(sizeof(DBCValDesc[3]) == sizeof(uintptr_t) * 3 * 2, "");
#endif
...
static bool DBCMachine_next2(DBCMachine* thiz, const DBCToken* token)
{
  ...
  case _UINTCAST:
    if (token->type == kUIntegerToken) {
      uinteger = cstrToUint32(token->data, &end, 10);
    }
    else if (token->type == kSIntegerToken) {
      uinteger = (uint32_t)cstrToInt32(token->data, &end, 10);
    }
    else {
      return false;
    }
  ...
  case _SINTEGER:
    if (token->type != kSIntegerToken && token->type != kUIntegerToken)
      return false;
    sinteger = cstrToInt32(token->data, &end, 10);
    if (token->data + token->size != end)
      return false;
  ...
  case _ENDIAN:
    if (token->type != kUIntegerToken)
      return false;
    DBC_ASSERT(token->size == 1);
    if (token->data[0] == '0')
      DBCMachine_pushUint32(thiz, kDBCBigEndian);
    else if (token->data[0] == '1')
      DBCMachine_pushUint32(thiz, kDBCLittleEndian);
    else
      return false;
    thiz->position++;
    return true;
  ...
  case _MPLEX:
    ...
    if (token->size == 1) {
      ...
      if (token->data[0] != 'm')
        return false;
      char* end;
      uint32_t value = cstrToUint32(token->data + 1, &end, 10);
      if (token->data + token->size == end)
  ...
  case _EXTYPE:
    if (token->type != kSymbolToken || token->size != 1)
      return false;
    if (token->data[0] == '0')
      DBCMachine_pushUint32(thiz, kDBCSigValueInteger);
    else if (token->data[0] == '1')
      DBCMachine_pushUint32(thiz, kDBCSigValueFloat);
    else if (token->data[0] == '2')
      DBCMachine_pushUint32(thiz, kDBCSigValueDouble);
    else if (token->data[0] == '3')
      DBCMachine_pushUint32(thiz, kDBCSigValueUnknown);
    else
      return false;
    thiz->position++;
    return true;
  ...
  case _EVTYPE:
    if (token->type != kSymbolToken || token->size != 1)
      return false;
    if (token->data[0] == '0')
      DBCMachine_pushUint32(thiz, kDBCEnvValueInteger);
    else if (token->data[0] == '1')
      DBCMachine_pushUint32(thiz, kDBCEnvValueFloat);
    else if (token->data[0] == '2')
      DBCMachine_pushUint32(thiz, kDBCEnvValueString);
    else
      return false;
    thiz->position++;
    return true;
  ...
  case _ACCESS:
    ...
    if (token->size <= 17 || strncmp(token->data, "DUMMY_NODE_VECTOR", 17) != 0)
      return false;
    uinteger = cstrToUint32(token->data + 17, &end, 16);
    if (token->data + token->size != end)
      return false;
    DBC_ASSERT((uinteger & ~(DBC_STRING_FLAG | DBC_READ_FLAG | DBC_WRITE_FLAG)) == 0);
    DBCMachine_pushUint32(thiz, uinteger);
    thiz->position++;
    return true;
...
{
  const struct DBCOpcode* opcode;
  DBC_ASSERT(!DBCMachine_isOverflow(thiz));
...
static bool DBCMachine_result(DBCMachine* thiz, DBCSection* section)
{
  if (thiz->done) {
...
struct DBCParserSection {
  char required;
  const char* keyword;
  int identifier;
  int errorCode;
  const struct DBCOpcode* entry;
  const struct DBCParserSection* childData;
  size_t childCount;
};

struct DBCParser {
  void* userdata;
  DBCParserCallback* callback;

  void* allocator;
  DBCAllocFn* alloc;
  DBCFreeFn* free;

  const struct DBCParserSection* parentSection;
  const struct DBCParserSection* currentSection;
...
static void initVersion(DBCMachine* thiz, DBCSection* section)
{
  section->version.type = kDBCSectionVersion;
  section->version.data = DBCMachine_shiftCString(thiz);
}

static void initNewSymbol(DBCMachine* thiz, DBCSection* section)
{
  section->newSymbols.type = kDBCSectionNewSymbols;
  section->newSymbols.data = DBCMachine_shiftCStringList(thiz, &section->newSymbols.count);
}

static void initBitTiming(DBCMachine* thiz, DBCSection* section)
{
  if (DBCMachine_empty(thiz))
    section->type = kDBCSectionEmpty;
  else {
    section->bitTiming.type = kDBCSectionBitTiming;
...
static void initNodes(DBCMachine* thiz, DBCSection* section)
{
  section->netNodes.type = kDBCSectionNetNodes;
  section->netNodes.names = DBCMachine_shiftCStringList(thiz, &section->netNodes.count);
}

static void initValueTable(DBCMachine* thiz, DBCSection* section)
{
  section->valTable.type = kDBCSectionValTable;
...
static void initMessage(DBCMachine* thiz, DBCSection* section)
{
  section->message.type = kDBCSectionMessage;
...
static void initSignalNormal(DBCMachine* thiz, DBCSection* section)
{
  section->signal.type = kDBCSectionSignal;
  ...
  section->signal.info.byteOrder = (DBCByteOrder)DBCMachine_shiftUint32(thiz);
...
static void initSignalMultiplexer(DBCMachine* thiz, DBCSection* section)
{
  section->signal.type = kDBCSectionSignal;
  section->signal.name = DBCMachine_shiftCIdentifier(thiz);
...
static void initMessageTransmitter(DBCMachine* thiz, DBCSection* section)
{
  section->messageTransmitter.type = kDBCSectionMessageTransmitter;
...
static void initEnvVar(DBCMachine* thiz, DBCSection* section)
{
  section->envVar.type = kDBCSectionEnvVar;
  section->envVar.name = DBCMachine_shiftCIdentifier(thiz);
  section->envVar.info.valueType = (DBCEnvValueType)DBCMachine_shiftUint32(thiz);
  ...
  section->envVar.info.accessType = (DBCEnvAccessType)DBCMachine_shiftUint32(thiz);
  section->envVar.accessNode = DBCMachine_shiftCStringList(thiz, &section->envVar.count);
}

static void initEnvVarData(DBCMachine* thiz, DBCSection* section)
{
  section->envVarData.type = kDBCSectionEnvVarData;
  section->envVarData.name = DBCMachine_shiftCIdentifier(thiz);
  section->envVarData.dataSize = DBCMachine_shiftUint32(thiz);
}

static void initSignalType(DBCMachine* thiz, DBCSection* section)
{
  section->signalType.type = kDBCSectionSignalType;
  section->signalType.name = DBCMachine_shiftCIdentifier(thiz);
  section->signalType.info.sizeInBits = DBCMachine_shiftUint32(thiz);
  section->signalType.info.byteOrder = (DBCByteOrder)DBCMachine_shiftUint32(thiz);
...
static void initNullComment(DBCMachine* thiz, DBCSection* section)
{
  section->comment.type = kDBCSectionComment;
  section->comment.target.objectType = kDBCObjectDocument;
  section->comment.value = DBCMachine_shiftCString(thiz);
  DBCMachine_clear(thiz);
}

static void initIgnoreComment(DBCMachine* thiz, DBCSection* section)
{
  section->comment.type = kDBCSectionEmpty;
  DBCMachine_clear(thiz);
}

static void initNetNodeComment(DBCMachine* thiz, DBCSection* section)
{
  section->comment.type = kDBCSectionComment;
  section->comment.target.objectType = kDBCObjectNetNode;
  section->comment.target.netNode.name = DBCMachine_shiftCIdentifier(thiz);
  section->comment.value = DBCMachine_shiftCString(thiz);
  DBCMachine_clear(thiz);
}

static void initMessageComment(DBCMachine* thiz, DBCSection* section)
{
  section->comment.type = kDBCSectionComment;
  section->comment.target.objectType = kDBCObjectMessage;
  section->comment.target.message.id = DBCMachine_shiftUint32(thiz);
  section->comment.value = DBCMachine_shiftCString(thiz);
}

static void initSignalComment(DBCMachine* thiz, DBCSection* section)
{
  section->comment.type = kDBCSectionComment;
  section->comment.target.objectType = kDBCObjectSignal;
...
static void initEnvVarComment(DBCMachine* thiz, DBCSection* section)
{
  section->comment.type = kDBCSectionComment;
  section->comment.target.objectType = kDBCObjectEnvVar;
  section->comment.target.envVar.name = DBCMachine_shiftCIdentifier(thiz);
  section->comment.value = DBCMachine_shiftCString(thiz);
  DBCMachine_clear(thiz);
}

static inline void initAttrDefineInt(DBCMachine* thiz, int type, DBCObjectType objectType, DBCSection* section)
{
  ...
  section->attrDefinition.value.type = kDBCAttrValueInt;
...
static inline void initAttrDefineHex(DBCMachine* thiz, int type, DBCObjectType objectType, DBCSection* section)
{
  section->attrDefinition.type = type;
  section->attrDefinition.objectType = objectType;
  section->attrDefinition.name = DBCMachine_shiftCString(thiz);
  section->attrDefinition.value.type = kDBCAttrValueHex;
...
static inline void initAttrDefineFloat(DBCMachine* thiz, int type, DBCObjectType objectType, DBCSection* section)
{
  section->attrDefinition.type = type;
  section->attrDefinition.objectType = objectType;
  section->attrDefinition.name = DBCMachine_shiftCString(thiz);
  section->attrDefinition.value.type = kDBCAttrValueFloat;
...
static inline void initAttrDefineString(DBCMachine* thiz, bool isRelation, DBCObjectType objectType, DBCSection* section)
{
  section->attrDefinition.type = isRelation ? kDBCSectionAttrRelDefinition : kDBCSectionAttrDefinition;
  section->attrDefinition.objectType = objectType;
  section->attrDefinition.name = DBCMachine_shiftCString(thiz);
  section->attrDefinition.value.type = kDBCAttrValueString;
...
static inline void initAttrDefineEnum(DBCMachine* thiz, int type, DBCObjectType objectType, DBCSection* section)
{
  section->attrDefinition.type = type;
  section->attrDefinition.objectType = objectType;
  section->attrDefinition.name = DBCMachine_shiftCString(thiz);
  section->attrDefinition.value.type = kDBCAttrValueEnum;
  section->attrDefinition.value.vEnum.data = DBCMachine_shiftCStringList(thiz, &section->attrDefinition.value.vEnum.count);
}

static void initAttrDefineNullInt(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineInt(thiz, kDBCSectionAttrDefinition, kDBCObjectDocument, section);
}

static void initAttrRelDefineNullInt(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineInt(thiz, kDBCSectionAttrRelDefinition, kDBCObjectDocument, section);
}

static void initAttrDefineNullHex(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineHex(thiz, kDBCSectionAttrDefinition, kDBCObjectDocument, section);
}

static void initAttrRelDefineNullHex(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineHex(thiz, kDBCSectionAttrRelDefinition, kDBCObjectDocument, section);
}

static void initAttrDefineNullFloat(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineFloat(thiz, kDBCSectionAttrDefinition, kDBCObjectDocument, section);
}

static void initAttrRelDefineNullFloat(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineFloat(thiz, kDBCSectionAttrRelDefinition, kDBCObjectDocument, section);
}

static void initAttrDefineNullString(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineString(thiz, false, kDBCObjectDocument, section);
}

static void initAttrRelDefineNullString(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineString(thiz, true, kDBCObjectDocument, section);
}

static void initAttrDefineNullEnum(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineEnum(thiz, kDBCSectionAttrDefinition, kDBCObjectDocument, section);
}

static void initAttrDefineNodeInt(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineInt(thiz, kDBCSectionAttrDefinition, kDBCObjectNetNode, section);
}

static void initAttrDefineNodeHex(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineHex(thiz, kDBCSectionAttrDefinition, kDBCObjectNetNode, section);
}

static void initAttrDefineNodeFloat(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineFloat(thiz, kDBCSectionAttrDefinition, kDBCObjectNetNode, section);
}

static void initAttrDefineNodeString(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineString(thiz, false, kDBCObjectNetNode, section);
}

static void initAttrDefineNodeEnum(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineEnum(thiz, kDBCSectionAttrDefinition, kDBCObjectNetNode, section);
}

static void initAttrDefineMessageInt(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineInt(thiz, kDBCSectionAttrDefinition, kDBCObjectMessage, section);
}

static void initAttrRelDefineMessageInt(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineInt(thiz, kDBCSectionAttrRelDefinition, kDBCObjectMessage, section);
}

static void initAttrDefineMessageHex(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineHex(thiz, kDBCSectionAttrDefinition, kDBCObjectMessage, section);
}

static void initAttrRelDefineMessageHex(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineHex(thiz, kDBCSectionAttrDefinition, kDBCObjectMessage, section);
}

static void initAttrDefineMessageFloat(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineFloat(thiz, kDBCSectionAttrDefinition, kDBCObjectMessage, section);
}

static void initAttrRelDefineMessageFloat(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineFloat(thiz, kDBCSectionAttrRelDefinition, kDBCObjectMessage, section);
}

static void initAttrDefineMessageString(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineString(thiz, false, kDBCObjectMessage, section);
}

static void initAttrRelDefineMessageString(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineString(thiz, true, kDBCObjectMessage, section);
}

static void initAttrDefineMessageEnum(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineEnum(thiz, kDBCSectionAttrDefinition, kDBCObjectMessage, section);
}

static void initAttrRelDefineMessageEnum(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineEnum(thiz, kDBCSectionAttrRelDefinition, kDBCObjectMessage, section);
}

static void initAttrDefineSignalInt(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineInt(thiz, kDBCSectionAttrDefinition, kDBCObjectSignal, section);
}

static void initAttrRelDefineSignalInt(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineInt(thiz, kDBCSectionAttrRelDefinition, kDBCObjectSignal, section);
}

static void initAttrDefineSignalHex(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineHex(thiz, kDBCSectionAttrDefinition, kDBCObjectSignal, section);
}

static void initAttrRelDefineSignalHex(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineHex(thiz, kDBCSectionAttrRelDefinition, kDBCObjectSignal, section);
}

static void initAttrDefineSignalFloat(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineFloat(thiz, kDBCSectionAttrDefinition, kDBCObjectSignal, section);
}

static void initAttrRelDefineSignalFloat(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineFloat(thiz, kDBCSectionAttrRelDefinition, kDBCObjectSignal, section);
}

static void initAttrDefineSignalString(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineString(thiz, false, kDBCObjectSignal, section);
}

static void initAttrRelDefineSignalString(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineString(thiz, true, kDBCObjectSignal, section);
}

static void initAttrDefineSignalEnum(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineEnum(thiz, kDBCSectionAttrDefinition, kDBCObjectSignal, section);
}

static void initAttrRelDefineSignalEnum(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineEnum(thiz, kDBCSectionAttrRelDefinition, kDBCObjectSignal, section);
}

static void initAttrDefineEnvVarInt(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineInt(thiz, kDBCSectionAttrDefinition, kDBCObjectEnvVar, section);
}

static void initAttrRelDefineEnvVarInt(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineInt(thiz, kDBCSectionAttrRelDefinition, kDBCObjectEnvVar, section);
}

static void initAttrDefineEnvVarHex(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineHex(thiz, kDBCSectionAttrDefinition, kDBCObjectEnvVar, section);
}

static void initAttrRelDefineEnvVarHex(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineHex(thiz, kDBCSectionAttrRelDefinition, kDBCObjectEnvVar, section);
}

static void initAttrDefineEnvVarFloat(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineFloat(thiz, kDBCSectionAttrDefinition, kDBCObjectEnvVar, section);
}

static void initAttrRelDefineEnvVarFloat(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineFloat(thiz, kDBCSectionAttrRelDefinition, kDBCObjectEnvVar, section);
}

static void initAttrDefineEnvVarString(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineString(thiz, false, kDBCObjectEnvVar, section);
}

static void initAttrRelDefineEnvVarString(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineString(thiz, true, kDBCObjectEnvVar, section);
}

static void initAttrDefineEnvVarEnum(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineEnum(thiz, kDBCSectionAttrDefinition, kDBCObjectEnvVar, section);
}

static void initAttrRelDefineEnvVarEnum(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefineEnum(thiz, kDBCSectionAttrRelDefinition, kDBCObjectEnvVar, section);
}

static void DBCMachineSetAttrValue(DBCMachine* thiz, DBCVariant* value)
{
  switch (DBCMachine_type(thiz)) {
  case kDBCValueUint32:
    value->type = kDBCVariantUint;
    value->vInt = (int64_t)DBCMachine_shiftUint32(thiz);
    break;
  case kDBCValueInt32:
    value->type = kDBCVariantInt;
    value->vInt = (int64_t)DBCMachine_shiftInt32(thiz);
    break;
  case kDBCValueCString:
    value->type = kDBCVariantString;
    value->vString = DBCMachine_shiftCString(thiz);
    break;
  default:
    value->type = kDBCVariantFloat;
    value->vFloat = DBCMachine_shiftDouble(thiz);
    break;
  }
}

static inline void initAttrDefaultBase(DBCMachine* thiz, int type, DBCSection* section)
{
...
static void initAttrDefault(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefaultBase(thiz, kDBCSectionAttrDefault, section);
}

static void initAttrRelDefault(DBCMachine* thiz, DBCSection* section)
{
  initAttrDefaultBase(thiz, kDBCSectionAttrRelDefault, section);
}

static void initAttrNull(DBCMachine* thiz, DBCSection* section)
{
  section->attr.type = kDBCSectionAttr;
  section->attr.name = DBCMachine_shiftCString(thiz);
  section->attr.target.objectType = kDBCObjectDocument;

  DBCMachineSetAttrValue(thiz, &section->attr.value);
}

static void initAttrNode(DBCMachine* thiz, DBCSection* section)
{
  section->attr.type = kDBCSectionAttr;
  section->attr.name = DBCMachine_shiftCString(thiz);
  section->attr.target.objectType = kDBCObjectNetNode;
  section->attr.target.netNode.name = DBCMachine_shiftCIdentifier(thiz);

  DBCMachineSetAttrValue(thiz, &section->attr.value);
}

static void initAttrMessage(DBCMachine* thiz, DBCSection* section)
{
  section->attr.type = kDBCSectionAttr;
  section->attr.name = DBCMachine_shiftCString(thiz);
  section->attr.target.objectType = kDBCObjectMessage;
  section->attr.target.message.id = DBCMachine_shiftUint32(thiz);

  DBCMachineSetAttrValue(thiz, &section->attr.value);
}

static void initAttrRelMessage(DBCMachine* thiz, DBCSection* section)
{
  section->attrRel.type = kDBCSectionAttrRel;
  section->attrRel.name = DBCMachine_shiftCString(thiz);
  section->attrRel.netNode = DBCMachine_shiftCIdentifier(thiz);
  section->attrRel.target.objectType = kDBCObjectMessage;
  section->attrRel.target.message.id = DBCMachine_shiftUint32(thiz);

  DBCMachineSetAttrValue(thiz, &section->attrRel.value);
}

static void initAttrSignal(DBCMachine* thiz, DBCSection* section)
{
  section->attr.type = kDBCSectionAttr;
  section->attr.name = DBCMachine_shiftCString(thiz);
  section->attr.target.objectType = kDBCObjectSignal;
...
static void initAttrRelSignal(DBCMachine* thiz, DBCSection* section)
{
  section->attrRel.type = kDBCSectionAttrRel;
  section->attrRel.name = DBCMachine_shiftCString(thiz);
  section->attrRel.netNode = DBCMachine_shiftCIdentifier(thiz);
  section->attrRel.target.objectType = kDBCObjectSignal;
...
static void initAttrEnvVar(DBCMachine* thiz, DBCSection* section)
{
  section->attr.type = kDBCSectionAttr;
  section->attr.name = DBCMachine_shiftCString(thiz);
  section->attr.target.objectType = kDBCObjectEnvVar;
  section->attr.target.envVar.name = DBCMachine_shiftCIdentifier(thiz);

  DBCMachineSetAttrValue(thiz, &section->attr.value);
}

static void initAttrRelEnvVar(DBCMachine* thiz, DBCSection* section)
{
  section->attrRel.type = kDBCSectionAttrRel;
  section->attrRel.name = DBCMachine_shiftCString(thiz);
  section->attrRel.netNode = DBCMachine_shiftCIdentifier(thiz);
  section->attrRel.target.objectType = kDBCObjectEnvVar;
  section->attrRel.target.envVar.name = DBCMachine_shiftCIdentifier(thiz);

  DBCMachineSetAttrValue(thiz, &section->attrRel.value);
}

static void initSignalTypeRef(DBCMachine* thiz, DBCSection* section)
{
  section->signalTypeRef.type = kDBCSectionSignalTypeRef;
...
static void initSignalGroup(DBCMachine* thiz, DBCSection* section)
{
  section->signalGroup.type = kDBCSectionSignalGroup;
...
static void initSignalValueType(DBCMachine* thiz, DBCSection* section)
{
  section->signalValueType.type = kDBCSectionSignalValueType;
  ...
  section->signalValueType.value = (DBCSigValueType)DBCMachine_shiftUint32(thiz);
}

static void initSignalValue(DBCMachine* thiz, DBCSection* section)
{
  section->signalValue.type = kDBCSectionSignalValue;
...
static void initValueEnvVar(DBCMachine* thiz, DBCSection* section)
{
  section->envVarValue.type = kDBCSectionEnvVarValue;
...
static void initExMultiplex(DBCMachine* thiz, DBCSection* section)
{
  section->signalMultiplexed.type = kDBCSectionSignalMultiplexed;
...
static const struct DBCParserSection s_message_child_sections[] = {
  // signals
  { '*', "SG_", DBC_KEYWORD_SG, kDBCErrorSectionSignal, s_signal, NULL },
  // comments
  { '*', "CM_", DBC_KEYWORD_CM, kDBCErrorSectionComment, s_comments, NULL },
};

static const struct DBCParserSection s_sections[] = {
  // version
  { '?', "VERSION", DBC_KEYWORD_VERSION, kDBCErrorSectionVersion, s_version, NULL, 0 },
  // new_symbols
  { '?', "NS_", DBC_KEYWORD_NS, kDBCErrorSectionNewSymbols, s_new_symbols, NULL, 0 },
  // bit_timing (*obsolete but required*)
  { '1', "BS_", DBC_KEYWORD_BS, kDBCErrorSectionBitTiming, s_bit_timing, NULL, 0 },
  // nodes
  { '1', "BU_", DBC_KEYWORD_BU, kDBCErrorSectionNetNodes, s_nodes, NULL, 0 },
  // value_tables
  { '*', "VAL_TABLE_", DBC_KEYWORD_VAL_TABLE, kDBCErrorSectionValTable, s_value_tables, NULL, 0 },
  // messages
  { '*', "BO_", DBC_KEYWORD_BO, kDBCErrorSectionMessage, s_messages, s_message_child_sections, ARRAY_SIZE(s_message_child_sections) },
  // message_transmitters
  { '*', "BO_TX_BU_", DBC_KEYWORD_BO_TX_BU, kDBCErrorSectionMessageTransmitter, s_message_transmitters, NULL, 0 },
  // environment_variables
  { '*', "EV_", DBC_KEYWORD_EV, kDBCErrorSectionEnvVar, s_environment_variables, NULL, 0 },
  // environment_variables_data
  { '*', "ENVVAR_DATA_", DBC_KEYWORD_ENVVAR_DATA, kDBCErrorSectionEnvVarData, s_environment_variables_data, NULL, 0 },
  // signal_types (*private*)
  { '*', "SGTYPE_", DBC_KEYWORD_SGTYPE, kDBCErrorSectionSignalType, s_signal_types, NULL, 0 },
  // comments
  { '*', "CM_", DBC_KEYWORD_CM, kDBCErrorSectionComment, s_comments, NULL, 0 },
  // attribute_definitions
  { '*', "BA_DEF_", DBC_KEYWORD_BA_DEF, kDBCErrorSectionAttrDefinition, s_attribute_definitions, NULL, 0 },
  // attribute_relation_definitions
  { '*', "BA_DEF_REL_", DBC_KEYWORD_BA_DEF_REL, kDBCErrorSectionAttrRelDefinition, s_attribute_relation_definitions, NULL, 0 },
  // sigtype_attr_list (*private*)
  // attribute_defaults
  { '*', "BA_DEF_DEF_", DBC_KEYWORD_BA_DEF_DEF, kDBCErrorSectionAttrDefault, s_attribute_defaults, NULL, 0 },
  // attribute_relation_defaults
  { '*', "BA_DEF_DEF_REL_", DBC_KEYWORD_BA_DEF_DEF_REL, kDBCErrorSectionAttrRelDefault, s_attribute_relation_defaults, NULL, 0 },
  // attribute_values
  { '*', "BA_", DBC_KEYWORD_BA, kDBCErrorSectionAttrValue, s_attribute_values, NULL, 0 },
  // attribute_relation_values
  { '*', "BA_REL_", DBC_KEYWORD_BA_REL, kDBCErrorSectionAttrRelValue, s_attribute_relation_values, NULL, 0 },
  // value_descriptions
  { '*', "VAL_", DBC_KEYWORD_VAL, kDBCErrorSectionValueDescription, s_value_descriptions, NULL, 0 },
  // category_definitions (*obsolete*)
  // categories (*obsolete*)
  // filter (*obsolete*)
  // signal_type_refs (*private*)
  // TODO: Del { '*', "SGTYPE_", DBC_KEYWORD_SIG_TYPE_REF, kDBCErrorSectionSignalTypeRef, s_signal_type_refs, NULL, 0 },
  { '*', "SIG_TYPE_REF_", DBC_KEYWORD_SIG_TYPE_REF, kDBCErrorSectionSignalTypeRef, s_signal_type_refs, NULL, 0 },
  // signal_groups
  { '*', "SIG_GROUP_", DBC_KEYWORD_SIG_GROUP, kDBCErrorSectionSignalGroup, s_signal_groups, NULL, 0 },
  // signal_extended_value_type_list (*private*)
  { '*', "SIG_VALTYPE_", DBC_KEYWORD_SIG_VALTYPE, kDBCErrorSectionSignalValueType, s_signal_extended_value_type_list, NULL, 0 },
  // extended_multiplexing
  { '*', "SG_MUL_VAL_", DBC_KEYWORD_SG_MUL_VAL, kDBCErrorSectionMultiplexedSignal, extended_multiplexing, NULL, 0 },
};
...
static bool DBCParser_chooseSection(DBCParser* thiz, const char* tokenData, size_t tokenSize)
{
  ...
  if (thiz->parentSection) {
    for (size_t index = 0; index < thiz->parentSection->childCount; index++) {
      const struct DBCParserSection* iter = &thiz->parentSection->childData[index];
  ...
  for (uint8_t index = thiz->sel; index < ARRAY_SIZE(s_sections); index++) {
    const struct DBCParserSection* iter = &s_sections[index];
    if (isKeywordEqual(iter->keyword, tokenData, tokenSize)) {
      thiz->sel = index;
      thiz->currentSection = iter;
  ...
  // DBC_LOGW("Keyword '%.*s' disrupts the sequence of the document", (int)tokenSize, tokenData);
...
static int DBCParser_next(DBCParser* thiz, const char** start, const char* end)
{
  int ret;
  bool repeat;
  DBCToken token;
  ...
  case kLexerNumberError:
    return kDBCErrorTokenNumber;

  case kLexerStringError:
    return kDBCErrorTokenString;

  case kLexerBufferError:
    return kDBCErrorBufferOverflow;

  default:
    return kDBCErrorInternal;
  }
  ...
  do {
    if (!thiz->hasMachine) {
      ...
      if (token.type != kCIdentifierToken)
        return kDBCErrorSectionKeyword;

      if (!DBCParser_chooseSection(thiz, token.data, token.size)) {
        return kDBCErrorSectionKeyword;
      }
    ...
    if (section.type != kDBCSectionEmpty && thiz->callback && !thiz->callback(thiz->userdata, &section))
      return kDBCErrorUser;
...
static void DBCParser_init(DBCParser* thiz, void* userdata, DBCParserCallback* callback, void* allocator, DBCAllocFn* alloc, DBCFreeFn* free)
{
...
{
  return DBCParser_create2(userdata, callback, NULL, &DBCAllocDefault, &DBCFreeDefault);
}
...
DBCParser* DBCParser_create2(void* userdata, DBCParserCallback* callback, void* allocator, DBCAllocFn* alloc, DBCFreeFn* free)
{
  ...
  DBCParser* thiz = (DBCParser*)alloc(allocator, sizeof(struct DBCParser));
  if (thiz == NULL)
    return NULL;

  DBCParser_init(thiz, userdata, callback, allocator, alloc, free);

  return thiz;
}

void DBCParser_destroy(DBCParser* thiz)
{
...
bool DBCParser_append(DBCParser* thiz, const char* data, size_t size)
{
  const char* start = data;
  const char* end = data + size;

  while (thiz->errorCode == 0 && start < end)
    thiz->errorCode = DBCParser_next(thiz, &start, end);

  return thiz->errorCode == 0;
}

bool DBCParser_finish(DBCParser* thiz)
{
  if (thiz->errorCode == 0)
    thiz->errorCode = DBCParser_next(thiz, NULL, NULL);
